...
async function fetchCarousel(url) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data?.detail || data?.error || "Erreur lors du chargement TMDB");
  }
  return res.json();
}
...
async function initCarousels() {
  ...
  document.querySelectorAll(".carousel").forEach((carousel) => {
    ...
    (async () => {
      try {
        const data = await fetchCarousel(url);
        if (!data) return;
        row.innerHTML = "";
        (data.results || []).forEach((item) => row.appendChild(createMovieCard(item, mediaType)));
      } catch (err) {
        console.error(err);
